...
globals [ total-population current-amount-of-states total-Militarycapabilities total-GDP total-area Hegemon power-transition war-counter]
...
to update-attributes ;; endogenous growth of variables since last tick
  ...
  ask states [ ;; update variables with endogeneous growth, set total values of import and export, set internal resource allocations
    ...
    let compensation-current-size militarycapabilities / GDP
    let increase 0
    if ( Decision-spending = true ) [ set increase increased-mil-spending set Decision-spending false]
    set MilitarySpending normal * (senseofsecurity-effects - compensation-current-size + increase )
    set MilitaryCapabilities (MilitaryCapabilities * (1 + MilitarySpending))
  ...
  ask states [
    ...
    if (any? my-diplomaticrelations with [conflict = true]) [
      ...
      foreach target [
        ...
        let count-allies count alliance-neighbors
        let count-target-allies count [alliance-neighbors] of ?
        let expected-spoils 0
        let expected-losses 0
        ifelse ( count-allies = 0 AND count-target-allies =  0) [
          set expected-spoils reparations-factor * count [my-provinces] of ? 
          set expected-losses reparations-factor * count my-provinces
        ]
        [ let powershare power / (power + sum[power] of alliance-neighbors)
          set expected-spoils (reparations-factor * (count [my-provinces] of ? + count [my-provinces] of [alliance-neighbors] of ?)) / powershare
          set expected-losses (reparations-factor * (count my-provinces + count [my-provinces] of alliance-neighbors )) / powershare
        ]
        ...
        ifelse ( [counter-round] of diplomaticrelation who [who] of ? = threshold-counter ) [
          let u-fight chance-winning * expected-spoils - expected-losses * (1 - chance-winning) - military-losses - loss-of-trade-direct + fight-security
          let u-submit submit-security + (reparations-factor - give-in-compensation) * count my-provinces
        ...
        [ ifelse (chance-winning  < 0.38) [ ;; you're screwed, your only option is to get a major power to defend you
          ifelse ( potential-allies != [] ) [ 
            ;; what conditions are present for forging an alliance?
            ...
            ask diplomaticrelation [who] of enemy [who] of alliance-choice [ set conflict true ]
          ]   
        ...
        [
          ifelse (chance-winning > 0.62)
            [
              ...
              let u-fight chance-winning * expected-spoils - expected-losses * (1 - chance-winning) - military-losses - loss-of-trade-direct + fight-security
              let u-nothing nothing-security
            ...
            [
              ...
              if ( alliance-choice != nobody) [
                ...
                set u-alliance-fight max-chance-winning * expected-spoils2 - expected-losses2 * (1 - max-chance-winning) - military-losses2 - loss-of-trade-direct2 + fight-security]
              ...
              if ( u-submit = max-utility ) [ask diplomaticrelation who [who] of ? [ ifelse (first-state = end1) [set decision-first "submit"][set decision-second "submit"]]]
              ...
              if ( u-alliance-fight = max-utility ) [
                create-alliance-with alliance-choice [ set alliance-target enemy ]
                ask diplomaticrelation [who] of enemy [who] of alliance-choice [ set conflict true 
                  ifelse (alliance-choice = end1) [set decision-first "mobilise & fight"][set decision-second "mobilise & fight"]]
                ask diplomaticrelation who [who] of enemy [ ifelse (first-state = end1) [set decision-first "mobilise & fight"][set decision-second "mobilise & fight"]]
              ]]]]]
      ]]
end
...
to interact
  ask diplomaticrelations [
    if (conflict = true)[
      if ( decision-first = "mobilise & fight" AND decision-second = "mobilise & fight" ) [ wage-war end1 end2 ];; show "1"]
      if ( decision-first = "submit" AND decision-second = "submit") [ set conflict false set type-of-last-conflict "submitted" set decision-first 0 set decision-second 0];; show "2"]
      ...
      if ( decision-first = "mobilise & fight" AND decision-second = "militarycapabilities") [ ;; states now have the choice: fight or submit
        ...
        [ ask end2 [set decision "mobilise & fight"]] [ask end2 [set decision "submit" ]]];; show "5" ] 
      ...
      if (decision-second = "mobilise & fight" AND decision-first = "militarycapabilities" ) [ 
        ...
        [ ask end1 [set decision "mobilise & fight"]] [ ask end1 [set decision "submit" ]]];; show "6"]
...
to wage-war [ #first #second ];; fight wars etc.
  ...
  let victim-power [MilitaryCapabilities ] of #second 
  let victim-alliance [alliance-neighbors] of #second
  let victim-alliance-power sum [MilitaryCapabilities  ] of victim-alliance
  
  let self-power [MilitaryCapabilities ] of #first
  let self-alliance [alliance-neighbors] of #first
  let self-alliance-power sum [MilitaryCapabilities ] of self-alliance
  
  if ((self-power + self-alliance-power) != 0 AND (victim-power + victim-alliance-power) != 0) 
  [
    set pWinningFirst (1 / ( 1 + exp(-((self-power + self-alliance-power) - (victim-power + victim-alliance-power)) /  ((self-power + self-alliance-power) + (victim-power + victim-alliance-power)))))
    set pWinningSecond (1 / ( 1 + exp(-((victim-power + victim-alliance-power) - (self-power + self-alliance-power)) /  ((self-power + self-alliance-power) + (victim-power + victim-alliance-power)))))
  ]
  ifelse ( pWinningFirst > pWinningSecond ) [ set winner #first set loser #second] [set winner #second set loser #first]
  show pWinningFirst show pWinningSecond
  
  ;; generate battle damage
  
  if (([major-power?] of #first = true OR [major-power?] of one-of self-alliance = true) AND ([major-power?] of #second = true OR [major-power?] of one-of victim-alliance = true)) [ show "major power war!!!!!!!!!!"]
   
  show ([militarycapabilities] of #first) show ([militarycapabilities] of self-alliance) show ([militarycapabilities] of #second) show ([militarycapabilities] of victim-alliance)
  ask #first [set militarycapabilities ( militarycapabilities - attritionrate * [militarycapabilities] of #second )
    if (MilitaryCapabilities < 0) [set MilitaryCapabilities 0]]
  ask self-alliance [set militarycapabilities ( militarycapabilities - attritionrate * [militarycapabilities] of #second )
    if (MilitaryCapabilities < 0) [set MilitaryCapabilities 0]]
  ask #second [set militarycapabilities ( militarycapabilities - attritionrate * [militarycapabilities] of #first )
    if (MilitaryCapabilities < 0) [set MilitaryCapabilities 0]]
  ask victim-alliance [set militarycapabilities ( militarycapabilities - attritionrate * [militarycapabilities] of #second )
    if (MilitaryCapabilities < 0) [set MilitaryCapabilities 0]]
  ...
  ask self-alliance [ ask diplomaticrelation who [who] of #second[ set decision-first 0 set decision-second 0 set type-of-last-conflict "armedconflict" set conflict false ]]
  ask victim-alliance [ ask diplomaticrelation who [who] of #second[ set decision-first 0 set decision-second 0 set type-of-last-conflict "armedconflict" set conflict false ]]
end 
...
to structural-change [ #winner #loser #type ] ;; update landscape according to the actions. If a war was fought and won, or bargaining process was done, may field different types of transfers of resources. 
  let location nobody                                            ;; the percentage of provinces that changes ownership is an input
  let spoilsize 0
  if (#type = "war") [ ;; this works for dyadic war, alliances are not taken into account
    set spoilsize round (reparations-factor * count patches with [my-state = #loser])]
  
  
  if (#type = "submit") [ ;; this works for dyadic war, alliances are not taken into account
    set spoilsize round ((reparations-factor - give-in-compensation) * [count my-provinces] of #loser)
  ]
  if (spoilsize = 0 ) [ set spoilsize 1]
  
  let self-alliance [alliance-neighbors] of #winner
  set self-alliance lput #winner self-alliance
  let victim-alliance [alliance-neighbors] of #loser 
  set victim-alliance lput #loser victim-alliance
  let total-power-self sum [power] of self-alliance
  let total-power-victim sum [power] of victim-alliance
   
  ;;let powershare power / (power + [power] of self-alliance)
  ;;let expected-spoils2 (reparations-factor * count [my-provinces] of #loser) / powershare
  ...
  while [spoilsize > 0 ] [
    ifelse (any? patches with [ my-state = #loser AND capital? = false and any? neighbors4 with [my-state = #winner]] )
      ...
      [ show "couldnt find any patches because only the capital remains" 
...
to-report find-senseofsecurity [#self #target #typeofchange] 
  let enemy-power 0
  let own-power [power * random-normal mean-Perception SD-perception ] of #self 
  let friendly-power own-power
  if ( hostile-states != 0 ) [
    foreach hostile-states [
      set enemy-power enemy-power + [power * random-normal mean-Perception SD-perception ] of ?
    ]]
  if ( friendly-states != 0 ) [
    foreach friendly-states [
      set friendly-power friendly-power + [power * random-normal mean-Perception SD-perception ] of ?
    ]]
  let initial-sense 0
...
to create-conflict ;; stochasticly create conflicts among states, with a randomised chance of a conflict for each relation, no matter alliances and attitudes
  ...
  let repeater random-pAttack * amount
  let repeater2 amount * interdependence-pAttack
