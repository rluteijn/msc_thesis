...
to update-attributes ;; endogenous growth of variables since last tick
  ...
  ask Exports [ 
    ...
    carefully [let growth mean [GDPpercapita-growth] of relevant-patches set tradevolume tradevolume * growth][ print self die ]
  ]  
  ...
  ask diplomaticrelations [
    if (conflict = true)[  
      ...
      if ( decision-first = "de-escalate" AND decision-second = "de-escalate" ) [ 
        set decision-first 0 set decision-second 0 set conflict-level conflict-level - 1 if (conflict-level < 1) [ set time-of-decision ticks set counter-round 0 record-conflict false end1 end2 set type-of-last-conflict "submitted" set decision-first 0 set decision-second 0]]
      ...
      if (decision-first = "de-escalate" AND decision-second = "escalate") [
        ...
        [ ifelse (decider = 2) 
          ...
          [set conflict-level conflict-level - 1 set decision-first 0 set decision-second 0 if (conflict-level < 1) [ set time-of-decision ticks set counter-round 0 record-conflict false end2 end1 set type-of-last-conflict "submitted" set decision-first 0 set decision-second 0]]
        ]]
      ...
      if ( decision-first = "de-escalate" AND decision-second != "escalate" AND decision-second != "de-escalate" AND decision-second != "nothing") [ 
        ifelse (decider = 1) [set conflict-level conflict-level - 1 set decision-first 0 set decision-second 0 if (conflict-level < 1) [ set time-of-decision ticks set counter-round 0 record-conflict false end2 end1 set type-of-last-conflict "submitted" set decision-first 0 set decision-second 0]]
        [set decision-first "submit"]]
      
      if ( decision-second = "de-escalate" AND decision-first != "escalate" AND decision-first != "de-escalate" AND decision-first != "nothing") [          
        ifelse (decider = 2) [set conflict-level conflict-level - 1 set decision-first 0 set decision-second 0 if (conflict-level < 1) [ set time-of-decision ticks set counter-round 0 record-conflict false end1 end2 set type-of-last-conflict "submitted" set decision-first 0 set decision-second 0]]
        [set decision-second "submit"]]
      ...
      if (decision-first = "escalate" AND decision-second = "submit") [
        ...
        [ ifelse (decider = 1) 
          [
            set decision-first 0 set decision-second 0  set conflict-level conflict-level + 1 
            if (conflict-level > 5) [ set conflict-level 5 set time-of-decision ticks set counter-round 0 record-conflict true end1 end2 wage-war end1 end2 set type-of-last-conflict "armedconflict"  set decision-first 0 set decision-second 0 ]]       
          [set conflict-level conflict-level - 1 set decision-first 0 set decision-second 0 if (conflict-level < 1) [ set time-of-decision ticks set counter-round 0 record-conflict false end1 end2 set type-of-last-conflict "submitted" set decision-first 0 set decision-second 0]]
        ]]
...
to record-conflict [ war #first #second ]
  ...
  ifelse ([Power > threshold-major-power] of #first AND [Power > threshold-major-power] of #second)[ 
    ...
    set type-conflict 1
    set-output war type-conflict #first #second multiple-MP1 multiple-MP2 highest-conflict-level-ever  
  ]
  [
    ifelse ([Power > threshold-major-power] of #first AND any? [alliance-neighbors with [Power > threshold-major-power]] of #second) [ 
      ...
      set type-conflict 2
      set-output war type-conflict #first relevant-state multiple-MP1 multiple-MP2 highest-conflict-level-ever
    ]    
    [
      ifelse (any? [alliance-neighbors  with [Power > threshold-major-power]] of #first AND [Power > threshold-major-power] of #second )[
        ...
        set multiple-MP2  (1 + counter2)  
        set type-conflict 2
        set-output war type-conflict #second relevant-state multiple-MP1 multiple-MP2 highest-conflict-level-ever
      ]
      [
        if ( any? [alliance-neighbors  with [Power > threshold-major-power]] of #first  AND any? [alliance-neighbors with [Power > threshold-major-power]] of #second)[
          ...
          set multiple-MP2  counter2  
          set type-conflict 3
          set-output war type-conflict relevant-state1 relevant-state2 multiple-MP1 multiple-MP2 highest-conflict-level-ever
        ]]]]
...
to set-output [ war type-conflict #first #second #counter1 #counter2 highest-conflict-level-ever]  
  if (ticks > memory-length) [
    if ( #first != nobody AND #second != nobody AND (diplomaticrelation [who] of #first [who] of #second) != nobody AND ticks > memory-length) [
      ...
      carefully[
        ...
        set max-conflict-level lput highest-conflict-level-ever max-conflict-level
