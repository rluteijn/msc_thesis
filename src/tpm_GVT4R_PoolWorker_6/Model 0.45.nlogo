...
to update-attributes ;; endogenous growth of variables since last tick
  ...
  ask one-of states [if (ticks = 1) [ show "new run"]]
  ask patches [
    ...
    let change-in_Population-growth 0.1 * change-in_GDPpercapita-growth
...
to wage-war [ #first #second ];; fight wars etc.
  ...
  if (([major-power?] of #first = true OR any? [alliance-neighbors  with [major-power? = true]] of #first ) AND ([major-power?] of #second = true OR any? [alliance-neighbors with [major-power? = true]] of #first)) [ 
    set major-power-war-counter major-power-war-counter + 1 show "major power war!!!!!!!!!!"]
...
to update-attitudes ;; if a war occured, or a bargaining process etc, attitudes are changed
  ask DiplomaticRelations [ ;; update attitudes
    ...
    ;; increase in attitude among allies, decrease in attitude between allies of both sides
...
to-report pWinning [ #self #victim #potentialally]
  ...
  carefully [set SD-perception-local(SD-perception * (1 / learning-effect) * percentage-hostile-states)] [ set SD-perception-local SD-perception]
    
  let victim-power [MilitaryCapabilities * random-normal mean-Perception SD-perception-local] of #victim 
  let victim-alliance [alliance-neighbors] of #victim
  let victim-alliance-power sum [MilitaryCapabilities * random-normal mean-Perception SD-perception-local ] of victim-alliance
  
  let self-power [MilitaryCapabilities * random-normal mean-Perception SD-perception-local] of #self
  if ( #potentialally != nobody ) 
  [ set self-power self-power + [MilitaryCapabilities] of #potentialally * random-normal mean-Perception SD-perception-local]
  let self-alliance [alliance-neighbors] of #self
  let self-alliance-power sum [MilitaryCapabilities * random-normal mean-Perception SD-perception-local] of self-alliance
...
to create-conflict ;; stochasticly create conflicts among states, with a randomised chance of a conflict for each relation, no matter alliances and attitudes
  ...
  if ( any? warmongers = true) [
    ...
    ask one-of warmongers [ 
      ...
      ask one-of my-diplomaticrelations with [ conflict = false] [ set conflict true]]
...
to foreign-policy-role
  ...
  if (sum [FPR-satisfaction] of states < total-dissatisfaction-threshold) [reset-FPR]
  ask states
  [ 
...
1375
647
total-dissatisfaction-threshold
-100
1
0
Number
